...
class BackgroundWorker(QThread):    # PyQt용 스레드
    initUiSignal = pyqtSignal(int)  # 스레드로 진행할 때 UI에서 초기에 처리할 초기화부분 시그널 전달
    ...
    def run(self) -> None:
        ...
        for i in range(maxVal):
            ...
            # self.parent.pgbTask.setValue(i) # UI 스레드의 권한을 백그라운드 스레드에게 주지 않음
...
class qtApp(QWidget):
    ...
    def btnStartClicked(self):
        ...
        th = BackgroundWorker(self) # qtApp 부모 클래스
