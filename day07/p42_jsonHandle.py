...
# with문으로 작업하면 fp.close() 할 필요가 없음
...
jData = json.dumps(data)    # json 

# json 데이터 접근은 파이썬 딕셔너리와 동일, 리스트와 동일하게 사용가능
print(data['name'])
...
with open('./day07/superCars.json', mode='w', encoding='utf-8') as fp:
    json.dump(superCars, fp, indent='\t', ensure_ascii=True) # 딕셔너리를 json 파일로 바꿔줌